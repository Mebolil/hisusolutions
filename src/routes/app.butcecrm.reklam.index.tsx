...
import { Plus, Search, Megaphone, Target, Pencil, Trash2, ChevronRight } from "lucide-react";
...
import {
  startOfMonth, endOfMonth, endOfWeek, subMonths,
...
function AdsPage() {
  ...
  // Gelir trendi — sadece kampanya bağlantılı satışlar
    ...
    if (period === "hafta") {
      return Array.from({ length: 7 }, (_, i) => {
        ...
        return {
          ...
          Gelir: sales.filter((s) => s.campaign_id && s.sale_date === day)
  ...
  async function toggleStatus(e: React.MouseEvent, c: Campaign) {
    e.stopPropagation();
    const { data: { session } } = await supabase.auth.getSession();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Kampanya Geliri" value={formatCurrency(stats.revenue)} valueClass="text-emerald-600" />
        <StatCard
          label="Genel ROAS"
      ...
      {enriched.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* Gelir trendi — kampanya kaynaklı */}
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-base">Kampanya Gelir Trendi</CardTitle>
                  <p className="text-xs text-muted-foreground mt-0.5">Kampanyaya bağlı satışlardan gelen gelir</p>
                </div>
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-1 items-center">
                          <Button size="icon" variant="ghost" className="h-8 w-8"
                            onClick={(e) => { e.stopPropagation(); setEditingCampaign(c); setEditOpen(true); }}>
                            <Pencil className="h-3.5 w-3.5" />
                          </Button>
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive hover:text-destructive"
                            onClick={(e) => { e.stopPropagation(); setDeletingCampaign(c); }}>
...
function NewCampaignDialog({
  ...
  function reset() {
    setForm({ name: "", platform: "", newPlatform: "", status: "aktif", spend: "0", budget: "", start_date: today, end_date: "" });
  }
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {form.platform === "__new__" && (
            <div>
              <Label>Yeni Platform Adı</Label>
              <Input value={form.newPlatform} onChange={(e) => setForm({ ...form, newPlatform: e.target.value })} required />
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
