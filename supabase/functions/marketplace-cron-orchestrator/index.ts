...
const STOCK_SYNC_INTERVAL_MS = 3.5 * 60 * 60 * 1000;   // 3.5 saat (4 saatte bir)
...
Deno.serve(async (req) => {
  ...
  const { data: connections, error } = await adminClient
    ...
    .select(
      "id, platform, last_order_sync_at, last_financial_sync_at, last_stock_sync_at, sync_status, sync_error_count, initial_backfill_done",
    )
    .eq("is_active", true)
    .is("deleted_at", null)
